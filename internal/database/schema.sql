...
CREATE TYPE public.task_project_status AS ENUM ('todo', 'in_progress', 'done');
--
...
CREATE TYPE public.team_member_role AS ENUM ('owner', 'member', 'guest');
--
...
CREATE FUNCTION public.not_empty(input text) RETURNS boolean LANGUAGE plpgsql STABLE AS $$ begin return (char_length(input) > 0);
end;
$$;
--
...
CREATE FUNCTION public.notify_after_notification_insert() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN PERFORM pg_notify('notification', row_to_json(NEW)::text);
...
CREATE TABLE public.schema_migrations (version character varying(128) NOT NULL);
--
    ...
    CONSTRAINT stripe_customers_only_one_reference_check CHECK (
        (
            (
                (user_id IS NOT NULL)
                AND (
                    customer_type = 'user'::public.stripe_customer_type
                )
                AND (team_id IS NULL)
            )
            OR (
                ...
                AND (
                    customer_type = 'team'::public.stripe_customer_type
                )
            )
        )
    )
...
CREATE TABLE public.tokens (
    ...
    CONSTRAINT tokens_type_identifier_token_not_empty CHECK (
        (
            public.not_empty(identifier)
            AND public.not_empty(token)
        )
    )
);
--
...
CREATE UNIQUE INDEX uniq_jobs_active_key ON public.jobs USING btree (unique_key)
WHERE (
        status = ANY (
            ARRAY ['pending'::public.job_status, 'processing'::public.job_status]
        )
    );
--
...
UPDATE ON public.app_params FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
-- Name: media handle_media_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER handle_media_updated_at BEFORE
UPDATE ON public.media FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.permissions FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.stripe_customers FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.stripe_prices FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.stripe_products FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.stripe_subscriptions FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.stripe_webhook_events FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.task_projects FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
-- Name: tasks handle_tasks_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER handle_tasks_updated_at BEFORE
UPDATE ON public.tasks FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.team_members FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
-- Name: teams handle_teams_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER handle_teams_updated_at BEFORE
UPDATE ON public.teams FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
-- Name: tokens handle_tokens_updated_at; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER handle_tokens_updated_at BEFORE
UPDATE ON public.tokens FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.user_accounts FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
UPDATE ON public.user_sessions FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();
--
...
CREATE TRIGGER trigger_notify_after_notification_insert
AFTER
INSERT ON public.notifications FOR EACH ROW EXECUTE FUNCTION public.notify_after_notification_insert();
--
...
ALTER TABLE ONLY public.media
ADD CONSTRAINT media_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.task_projects
ADD CONSTRAINT task_projects_assignee_id_fkey FOREIGN KEY (assignee_id) REFERENCES public.team_members(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.task_projects
ADD CONSTRAINT task_projects_created_by_member_id_fkey FOREIGN KEY (created_by_member_id) REFERENCES public.team_members(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.task_projects
ADD CONSTRAINT task_projects_reporter_id_fkey FOREIGN KEY (reporter_id) REFERENCES public.team_members(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.tasks
ADD CONSTRAINT tasks_assignee_id_fkey FOREIGN KEY (assignee_id) REFERENCES public.team_members(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.tasks
ADD CONSTRAINT tasks_created_by_member_id_fkey FOREIGN KEY (created_by_member_id) REFERENCES public.team_members(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.tasks
ADD CONSTRAINT tasks_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES public.tasks(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.tasks
ADD CONSTRAINT tasks_reporter_id_fkey FOREIGN KEY (reporter_id) REFERENCES public.team_members(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
...
ALTER TABLE ONLY public.team_members
ADD CONSTRAINT team_members_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE
SET NULL;
--
