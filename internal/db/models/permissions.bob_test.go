...
func TestPermissionUniqueConstraintErrors(t *testing.T) {
	...
	tests := []struct {
		...
		applyFn     func(tpl *factory.PermissionTemplate, obj *models.Permission)
	}{
		{
			name:        "ErrUniquePermissionsPkey",
			expectedErr: models.PermissionErrors.ErrUniquePermissionsPkey,
			applyFn: func(tpl *factory.PermissionTemplate, obj *models.Permission) {
				tpl.Apply(
					factory.PermissionMods.RandomizeAllColumns(nil),
					factory.PermissionMods.ID(obj.ID),
				)
			},
		},

		{
			name:        "ErrUniquePermissionsNameKey",
			expectedErr: models.PermissionErrors.ErrUniquePermissionsNameKey,
			applyFn: func(tpl *factory.PermissionTemplate, obj *models.Permission) {
				tpl.Apply(
					factory.PermissionMods.RandomizeAllColumns(nil),
					factory.PermissionMods.Name(obj.Name),
				)
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			exec := bob.New(tx)
			f := factory.New()
			tpl := f.NewPermission(factory.PermissionMods.RandomizeAllColumns(nil))
			obj, err := tpl.Create(ctx, exec)
			if err != nil {
				t.Fatal(err)
			}
			tt.applyFn(tpl, obj)
			_, err = models.Permissions.Insert(tpl.BuildSetter()).One(ctx, exec)
