...
const mockLedger = rpsGameQueries as unknown as {
...
describe("SubmitMoveView — insufficient funds", () => {
  ...
  it("shows all moves enabled when balance covers bet", async () => {
    ...
    render(<SubmitMoveView game={makeGame(100)} onOpenChange={vi.fn()} />);
    ...
    expect(
      screen.getByRole("button", { name: "Select a Move" }),
    ).toBeDisabled();
    // disabled only because no move selected, not because of funds
    expect(screen.queryByText(/buy points/i)).not.toBeInTheDocument();
  });
  ...
  it("shows insufficient funds warning when balance < bet", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /you need 100 pts to accept this bet but only have 10 pts/i,
        ),
      ).toBeInTheDocument();
    });
  ...
  it("submit button disabled due to insufficient funds even after selecting a move", async () => {
    ...
    render(<SubmitMoveView game={makeGame(50)} onOpenChange={vi.fn()} />);
  ...
  it("no warning shown when game has no bet", async () => {
    ...
    render(<SubmitMoveView game={makeGame(0)} onOpenChange={vi.fn()} />);
